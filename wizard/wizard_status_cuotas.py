...
class WizardStatusCuotas(models.TransientModel):
    ...
    lista_prestamos = fields.Many2one("loan.management.loan", "Prestamo Numero")
    ...
    def generar_status_cuotas(self, prestamo_id):
        ...
        if prestamo_id:
            # print "voy por uno"
            prestamos_obj = self.env["loan.management.loan"].search([('id', '=', self.lista_prestamos.id)])
...
class WizardGeneratePlanCuotas(models.TransientModel):
    """DCLS""" 
    _name = 'loan.wizard.generar.plan.cuotas'
    ...
    def generate_plan_cuotas(self):
        ...
        for record in self.env['loan.management.loan'].browse(active_ids):
            if record.state not in ('cotizacion'):
                raise UserError(_("Selected loan(s) cannot be confirmed as they are not in 'Aprobado' state."))
    ...
    def generate_approve(self):
        ...
        for record in self.env['loan.management.loan'].browse(active_ids):
            if record.state not in ('progress','desembolso'):
                raise UserError(_("Selected loan(s) cannot be confirmed."))
...
class WizardGenerarContabilidad(models.TransientModel):
    """DCLS""" 
    _name = 'loan.wizard.generate.accounting'
